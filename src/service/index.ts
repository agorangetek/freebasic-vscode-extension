...
import type { FbDocument, FbSymbol } from './types.ts';

export class FbIndex {
	private documents = new Map<string, FbDocument>();
	...
	index(uri: string, text: string): FbDocument {
	...
	get(uri: string): FbDocument | undefined {
	...
	symbols(excludeUri?: string): FbSymbol[] {
		const out: FbSymbol[] = [];
